...
//    }
    ...
    if(commande ==0){
      seulementMesure = false;
    }else if(!(commande==1 || commande == 2 || commande == 3)){ // si la commande est différente de 1,2 ou 3
      goto  remonte;
    }

    Serial2.println("ok\r");
    Serial.println("ok\r");

    if (!seulementMesure) {
      reinitialise();
      avancer_tick=action(commande,distance);
    }
...
String lecture(){
  String lire="";
  while (Serial2.available()) { // tant que des caractères sont en attente
      lire+=Serial2.read();
...
void prise(){
  while (true){
    ...
    String res = String(";distance;")+String(+distance)+String(";angle;")+String(angle)+";";
    //Serial.println(res);
    //Serial2.println(res);
    
    mesures[angle-20]=res;// on remplit le tableau
...
float action(int commande, int dist){ 
  /* variable commande : 
      1 : avancer
      2 : reculer
      3 : tour sens anti-horaire
      4 : tour sens horaire // en commentaire, car notre robot est plus précis en anti_horaire
  */
